...
import logo from "./../assets/web-icon.png";
    ...
    const getNotificationsCount = async () => {
      try {
        const response = await axios.get(
          `${BASE_URL}/notifications/getUnreadNotificationsOfUser`,
          {
            params: {
              userId: userId,
            },
  ...
  return (
    <>
      <nav className=" h-full p-2 hidden sm:flex flex-col px-3 overflow-y-auto overflow-x-hidden">
          ...
          {adminNavs.map((nav, index) => {
            return (
              <NavLink
                ...
                className={({ isActive }) =>
                  isActive
                    ? "group text-primary font-semibold text-md flex justify-start items-center gap-2 text-xl h-12 mb-4 activeNav"
              ...
              >
                <div className="inline-flex group-hover:bg-backgroundDark transition duration-300 ease-in-out justify-center items-center gap-0 xl:gap-3 px-5 h-12 rounded-full relative">
                  ...
                  <div className="leading-[48px]">
                    <i className="not-italic hidden xl:inline">{nav.text}</i>{" "}
                    {nav.text === "Notifications" && unReadCount > 0 && (
                      <div className="w-6 h-6 flex justify-center items-center bg-primary text-white absolute top-[-6px] right-[-6px] rounded-full font-normal text-sm">
                        {unReadCount}
                      </div>
                    )}
                  </div>
        ...
        <div className="mt-auto w-full rounded-full  min-h-16 basis-0 flex mb-2  justify-center items-center px-2 gap-2 hover:bg-backgroundDark cursor-pointer">
